...
generate_init_condi <- function(r0,
  ...
  ##
  E0 <- sum(daily_new_case_all[(feb26_idx+round(Dp)):(feb26_idx+round(Dp)+round(De)-1)]) / r0 
  P0 <- sum(daily_new_case_all[feb26_idx:(feb26_idx+round(Dp)-1)]) / r0                    
  I0 <- sum(daily_new_case_all[(feb26_idx-round(Di)):(feb26_idx-1)])   
  A0 <- I0 * (1 - r0) / r0
...
SEIRfitting=function(init_sets_list, 
  ...
  # write.table(s, paste0("...../Start_period_days_for",run_id,".txt"), quote = F, row.names = F, sep = "\t")
  
  #in order to get the same graph
  s<-c(24,127,197,208,240)
  
  
  startValue1<-pars_sampler()
  while (is.infinite(loglh_func_before_fixed_periods(pars=startValue1,s))) {startValue1=pars_sampler()}
  
  
  m<-matrix(rep(s, n_iterations), ncol=5,byrow=T)
  
  for (r in 2:n_iterations)
  {
    v<-m[r-1,]
    ...
    if (is.infinite(loglh_func_before_fixed_periods (startValue1,proposed_step1)) ) {proposed_step1<- v}
    ratio<-loglh_func_before_fixed_periods (startValue1,proposed_step1)- loglh_func_before_fixed_periods (startValue1,v) 
    a<- min(1, exp(ratio))
    m[r,]<-m[r-1,]
    if( runif(1)<=a ){m[r,]<-proposed_step1}
  }
  start_of_periods <- round(c(mean(m[,1]),mean(m[,2]),mean(m[,3]),mean(m[,4]),mean(m[,5])))
  
  write.table(m, paste0("...../perd_est_run_",run_id,".txt"), quote = F, row.names = F, sep = "\t")
  
  png(paste0("...../par_hist_period_run_",run_id,".png"))
  par(mfrow = c(2, 3))
  ...
  dev.off() 
  png(paste0("...../par_traj_period_run_",run_id,".png"), width=1000, height=500)
  par(mfrow = c(2, 3))
  for(i in 1:5) {
  ...
  if (!skip_MCMC) {
    ...
    write.table(startValue, paste0("...../Start_value_for",run_id,".txt"), quote = F, row.names = F, sep = "\t")
    ...
    write.table(crit,row.names=c("AIC", "BIC"), paste0("...../criterion",run_id,".txt"))
...
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep3", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep4", output_ret = T, skip_MCMC=F)