...
generate_init_condi <- function(r0,
                                ...
                                Dh = 30,
                                N = 8600000,
                                flowN = c(339395,340112, 341185,341185)
) {
  ...
  daily_new_case_all<- rep(realData_all[1],244)
  for (i in 2:244) 
  {
    daily_new_case_all[i] <-realData_all[i]- realData_all[i-1]
  }
  #smoothing
  d<-ksmooth(c(1:244), daily_new_case_all, "normal", bandwidth = 7)
  ...
  transform_var_main_4stage=function(pars) {
    ...
    ##
    r1 <- pars[5]
    r2 <- 1 / (1 + (1 - r1) / (r1 * exp(pars[6])))
    r3 <- 1 / (1 + (1 - r2) / (r2 * exp(pars[7])))
    r4 <- 1 / (1 + (1 - r3) / (r3 * exp(pars[8])))
    r_vec <- c(r1,r2,r3,r4)
  ...
  return(list(Di=Di,
              ...
              par_lower = c(b1= 0, b2 = 0, b3 = 0, b4 = 0, r1 = 0, delta3 = -10, delta4 = -10, delta5 = -10),
              par_upper = c(b1 = 2, b2 = 2, b3 = 2, b4 = 2, r1 = 1, delta3 = 10, delta4 = 10, delta5 = 10)))
...
default_pars_density <- function(pars) {
  ...
  ## r1
  r1 <- pars[5]
  d_vec[5] <- dbeta(r1, beta_shape1, beta_shape2, log = T)
  ## r2
  delta_3 <- pars[6]
...
SEIRfitting=function(init_sets_list, 
                     ...
                     pars_name=c("b1", "b2", "b3", "b4", "r1", "delta3", "delta4", "delta5"),
                     calc_clearance=F,
                     n_burn_in=0,
                     n_iterations=20000) {
...
Findzero <- function(pars_estimate, init_settings) {
  ...
  ##
  est_mat <- apply(pars_estimate, 1, function(x) SEIRsimu(pars = x, init_settings = init_settings, num_periods = 4)[, c("I", "P", "E", "A")])
...
SEIRsimu <- function(pars, init_settings, num_periods = 4) {
  tmp_ret=init_settings$var_trans_fun(pars)
  b_vec=tmp_ret[[1]]
  r_vec=tmp_ret[[2]]
  ...
  stage_start <- c(startp1, startp2, startp3, startp4)               # corresponding to dates ...
  stage_end <- c(endp1, endp2, endp3,length(days_to_fit))    # corresponding to dates Jan9, Jan22, Feb1, Feb16, the last day (could vary)
  ##
  myold_states <- init_states
  for (i_stage in 1:4) {
    stage_pars_setings <- c(b = b_vec[i_stage], r = r_vec[i_stage], Dq = Dq_vec[i_stage], n = flowN_vec[i_stage])
...
SEIRplot <- function(pars_estimate, file_name, init_settings, panel_B_R_ylim=6) {
  ...
  ##
  init_settings$days_to_fit <- 1:237
  library(vioplot)
  ##
  ##
  ##
  ...
  axis(1, at = seq(1, 240, 60), labels = mydate[seq(1, 235, 60)])
  #
  abline(v = c(startp2,startp3,startp4, 235), lty = 3, lwd = 2, col = "darkgrey")
  text(c(startp2,startp3,startp4, 235), par()$usr[4], labels = mydate[c(startp2,startp3,startp4, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
  polygon(c(ptime[1:235], rev(ptime[1:235])), c(estN_up[1:235], rev(estN_low[1:235])), col = "#F39B7FB2", border = NA)
  polygon(c(ptime[-c(1:234)], rev(ptime[-c(1:234)])), c(estN_up[-c(1:234)], rev(estN_low[-c(1:234)])), col = "#4DBBD5B2", border = NA)
  #
  points(ptime[1:234], estN_mean[1:234], col = "#BC3C29FF", pch = 16, cex = 0.8)
  points(ptime[-c(1:234)], estN_mean[-c(1:234)], col = "#0072B5FF", pch = 17, cex = 0.8)
  ##
  ##
  ##
  ...
  vioplot(estRt_mat[, 1], estRt_mat[, 2], estRt_mat[, 3], estRt_mat[, 4], names = NA, ylim = c(-1.5, panel_B_R_ylim),col = c("#BC3C29FF","#0072B5FF", "#E18727FF", "#7876B1FF", "#FFDC91FF"), ylab = "", xlab = "")
  mtext("Outbreak period (2020)", side = 1, line  = 3, cex = 1.01)
  mtext(expression("R"["0"]), side = 2, line = 3, cex = 1.01)
  axis(1, at = c(1, 2, 3, 4), tick = F, labels = c("March 3-March 17", "March 18-May 12", "May 13-Sept 13", "Sept 14-Oct 25"))
  ...
  mtext("Onset date (2020)", side = 1, line  = 3, cex = 1.01)
  mtext("No. of ascertained cases", side = 2, line = 3, cex = 1.01)
  axis(1, at = seq(1, 240, 60), labels = mydate[seq(1, 240, 60)])
  #
  abline(v = c(startp2,startp3,startp4, 235), lty = 3, lwd = 2, col = "darkgrey")
  text(c(startp2,startp3,startp4, 235), par()$usr[4], labels = mydate[c(startp2,startp3,startp4, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
  ...
  points(ptime[-c(1:endp3)], estN_mean[-c(1:endp3)], col = "#0072B5FF", pch = 17, cex = 0.8)
  ##
  ##
  ##
  # points(ptime, onset_obs_all, col = "black", pch = 4, cex = 0.8)
  ...
  points(ptime[-c(1:endp2)], estN_mean[-c(1:endp2)], col = "#0072B5FF", pch = 17, cex = 0.8)
  ##
  ##
  ##
  # points(ptime, onset_obs_all, col = "black", pch = 4, cex = 0.8)
  ...
  ##########################################   Panel E  ##########################################################
  estN_mat <- apply(pars_estimate, 1, function(x) SEIRsimu(pars = x, init_settings = init_settings, num_periods = 1)[, "Onset_expect"])
  estN_mean <- round(apply(estN_mat, 1, mean), 0)
  estN_up <- round(apply(estN_mat, 1, function(x) quantile(x, 0.975)), 0)
  estN_low <- round(apply(estN_mat, 1, function(x) quantile(x, 0.025)), 0)
  # start E
  ...
  text(c(startp2,startp3,startp4, 235), par()$usr[4], labels = mydate[c(startp2,startp3,startp4, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
  polygon(c(ptime[1:startp2], rev(ptime[1:startp2])), c(estN_up[1:startp2], rev(estN_low[1:startp2])), col = "#F39B7FB2", border = NA)
  polygon(c(ptime[-c(1:endp1)], rev(ptime[-c(1:endp1)])), c(estN_up[-c(1:endp1)], rev(estN_low[-c(1:endp1)])), col = "#4DBBD5B2", border = NA)
  #
  points(ptime[1:endp1], estN_mean[1:endp1], col = "#BC3C29FF", pch = 16, cex = 0.8)
  points(ptime[-c(1:endp1)], estN_mean[-c(1:endp1)], col = "#0072B5FF", pch = 14, cex = 0.8)
  ##
  ##
  ##
  # points(ptime, onset_obs_all, col = "black", pch = 4, cex = 0.8)
  ...
  ##########################################   Panel F  ##########################################################
  estAIP_mat <- apply(pars_estimate, 1, function(x) SEIRsimu(pars = x, init_settings = init_settings, num_periods = 4)[, c("I", "A", "P")])
  ...
  axis(1, at = barpos[seq(1, 240, 60)], labels = mydate[seq(1, 240, 60)])
...
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep5", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep6", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep7", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep8", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep9", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep10", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep11", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep12", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep13", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep14", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep15", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep16", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep17", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep18", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep19", output_ret = T, skip_MCMC=F)
SEIRfitting(init_sets_list, randomize_startValue = T, run_id = "main_analysis_rep20", output_ret = T, skip_MCMC=F)
