...
generate_init_condi <- function(r0,
                                ...
                                Dh = 30,
                                N = 8600000,
                                flowN = c(339395,340112, 341185,3411850)
) {
...
SEIRfitting=function(init_sets_list, 
  ...
  ##1 these files are created to locate if some errors can are found
  u<-'u'
...
SEIRsimu <- function(pars, init_settings, num_periods = 4) {
  ...
  stage_start <- c(1, 15, 77, 201)               # corresponding to dates ...
  stage_end <- c(14, 76, 200,length(days_to_fit))    # corresponding to dates Jan9, Jan22, Feb1, Feb16, the last day (could vary)
  ##
  myold_states <- init_states
  for (i_stage in 1:4) {
    stage_pars_setings <- c(b = b_vec[i_stage], r = r_vec[i_stage], Dq = Dq_vec[i_stage], n = flowN_vec[i_stage])
...
SEIRplot <- function(pars_estimate, file_name, init_settings, panel_B_R_ylim=6) {
 ...
 # plot(ptime, estN_mean, ylim = c(0, max(estN_up, onset_obs_all) * 1.05), xlab = "", ylab = "", type = "p", col = "white", pch = 16, xaxt="n", cex = 0.5)
  ...
  #
  abline(v = c(15,77,201, 235), lty = 3, lwd = 2, col = "darkgrey")
  text(c(15,77,201, 235), par()$usr[4], labels = mydate[c(15,77,201, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
 ...
 # plot(ptime, estN_mean, ylim = c(0, max(estN_up, onset_obs_all) * 1.05), xlab = "", ylab = "", type = "p", col = "white", pch = 16, xaxt="n", cex = 0.5)
  ...
  text(c(15,77,201, 235), par()$usr[4], labels = mydate[c(15,77,201, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
  polygon(c(ptime[1:201], rev(ptime[1:201])), c(estN_up[1:201], rev(estN_low[1:201])), col = "#F39B7FB2", border = NA)
  polygon(c(ptime[-c(1:200)], rev(ptime[-c(1:200)])), c(estN_up[-c(1:200)], rev(estN_low[-c(1:200)])), col = "#4DBBD5B2", border = NA)
  #
  points(ptime[1:200], estN_mean[1:200], col = "#BC3C29FF", pch = 16, cex = 0.8)
  points(ptime[-c(1:200)], estN_mean[-c(1:200)], col = "#0072B5FF", pch = 17, cex = 0.8)
  ##
  ##
  ##
   # points(ptime, onset_obs_all, col = "black", pch = 4, cex = 0.8)
...
#  plot(ptime, estN_mean, ylim = c(0, max(estN_up, onset_obs_all) * 1.01), xlab = "", ylab = "", type = "p", col = "white", pch = 16, xaxt="n", cex = 0.5)
  ...
  text(c(15,77,201, 235), par()$usr[4], labels = mydate[c(15,77,201, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
  polygon(c(ptime[1:77], rev(ptime[1:77])), c(estN_up[1:77], rev(estN_low[1:77])), col = "#F39B7FB2", border = NA)
  polygon(c(ptime[-c(1:76)], rev(ptime[-c(1:76)])), c(estN_up[-c(1:76)], rev(estN_low[-c(1:76)])), col = "#4DBBD5B2", border = NA)
  #
  points(ptime[1:76], estN_mean[1:76], col = "#BC3C29FF", pch = 16, cex = 0.8)
  points(ptime[-c(1:76)], estN_mean[-c(1:76)], col = "#0072B5FF", pch = 17, cex = 0.8)
  ##
  ##
  ##
  # points(ptime, onset_obs_all, col = "black", pch = 4, cex = 0.8)
  ...
  text(c(15,77,201, 235), par()$usr[4], labels = mydate[c(15,77,201, 235)], col = "darkgrey", pos = 3, xpd = T)
  #
  polygon(c(ptime[1:15], rev(ptime[1:15])), c(estN_up[1:15], rev(estN_low[1:15])), col = "#F39B7FB2", border = NA)
  polygon(c(ptime[-c(1:14)], rev(ptime[-c(1:14)])), c(estN_up[-c(1:14)], rev(estN_low[-c(1:14)])), col = "#4DBBD5B2", border = NA)
  #
  points(ptime[1:14], estN_mean[1:14], col = "#BC3C29FF", pch = 16, cex = 0.8)
  points(ptime[-c(1:14)], estN_mean[-c(1:14)], col = "#0072B5FF", pch = 14, cex = 0.8)
...
#1.322	0.207	0.439	1.912	0.779	1.818	-0.468	-0.274

startv=c(1.322,	0.207,	0.439	,1.912,	0.779,	1.818	,-0.468	,-0.274)
startValue<-matrix(NA, 1,8)
