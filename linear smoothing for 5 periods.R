...
generate_init_condi <- function(r0,
  ...
  return(list(Di=Di,
              ...
              par_lower = c(b1= 0, b2 = 0, b3 = 0, b4 = 0, b5 = 0,  r1 = 0, delta3 = -10, delta4 = -10, delta5 = -10, delta6 = -10, pshift1=-3,pshift2=-3,pshift3=-3,pshift4=-3),
              par_upper = c(b1 = 2, b2 = 2, b3 = 2, b4 = 2,b5 = 2,  r1 = 1, delta3 = 10, delta4 = 10, delta5 = 10, delta6 = 10,pshift1=3,pshift2=3,pshift3=3,pshift4=3)))
...
SEIRfitting=function(init_sets_list, 
                     ...
                     pars_name=c("b1", "b2", "b3", "b4","b5","r1", "delta3", "delta4", "delta5","delta6", "shift1", "shift2","shift3","shift4"),
                     calc_clearance=F,
                     n_burn_in=0,
                     n_iterations=10000) {
...
SEIRpred <- function(pars, init_settings) {
  ...
  stage_intervals=list(
    ...
    c(start=round(24+shift_vec[1]), end=round(100+shift_vec[2])),
    c(start=round(101+shift_vec[2]), end=round(195+shift_vec[3])),
    c(start=round(196+shift_vec[3]), end=round(241+shift_vec[4])),
    c(start=round(242+shift_vec[4]), end=255))
...
SEIRsimu <- function(pars, init_settings, num_periods = 5) {
  ...
  ##
  startp1<-1
  endp1<-round(23+shift_vec[1])
  
  startp2<- round(24+shift_vec[1])
  endp2<-round(100+shift_vec[2])
  
  startp3<-round(101+shift_vec[2])
  endp3<-round(195+shift_vec[3])
  
  startp4<-round(196+shift_vec[3])
  endp4<-round(241+shift_vec[4])
  
  startp5<-round(242+shift_vec[4])
  endp5<-255
...
SEIRplot <- function(pars_estimate, file_name, init_settings, panel_B_R_ylim=5) {
  ...
  shift_vec<-rep(NA,4)
  for ( i in 11:14)
  {shift_vec[i-10]<-pars_estimate[,i]}
