...
generate_init_condi <- function(r0,
 ...
 #cases for the 25th of February
  ...
  transform_var_main_4stage=function(pars) {
    ...
    return(list(b_vec, r_vec,p_shift))
  }
  ...
  return(list(Di=Di,
              ...
              par_lower = c(b1= 0, b2 = 0, b3 = 0, b4 = 0, r1 = 0, delta3 = -10, delta4 = -10, delta5 = -10,pshift1=-3,pshift2=-3,pshift3=-3),
              par_upper = c(b1 = 2, b2 = 2, b3 = 2, b4 = 2, r1 = 1, delta3 = 10, delta4 = 10, delta5 = 10,pshift1=3,pshift1=3,pshift1=3)))
...
default_pars_density <- function(pars) {
  ...
  ## shifts
  for(i in c(9:11)) {
    d_vec[i] <- dunif(pars[i], -2, 2, log = T)
  }
  return(sum(d_vec))
}

default_pars_sampler <- function(n = 1) {
  s_vec <- matrix(NA, n,11)
  ## b1,b2, b3, b4
  for(i in c(1:4)) {
    s_vec[, i] <- runif(n, 0, 2) 
  }
...
SEIRfitting=function(init_sets_list, 
                     ...
                     pars_name=c("b1", "b2", "b3", "b4", "r1", "delta3", "delta4", "delta5", "shift1", "shift2","shift3"),
                     calc_clearance=F,
                     n_burn_in=0,
                     n_iterations=20000) {
  ...
  ##1 files to identify in cases there are errors
  u<-'u'
...
SEIRpred <- function(pars, init_settings) {
  ...
  shift_vec=tmp_ret[[3]]
  stage_intervals=list(
    c(start=1, end=round(21+shift_vec[1])),
    c(start=round(22+shift_vec[1]), end=round(76+shift_vec[2])),
    c(start=round(77+shift_vec[3]), end=round(200+shift_vec[3])),
    c(start=round(201+shift_vec[3]), end=255))
...
SEIRsimu <- function(pars, init_settings, num_periods = 4) {
  ...
  ##
  startp1<-1
  endp1<-round(21+shift_vec[1])
  startp2<- round(22+shift_vec[1])
  endp2<-round(76+shift_vec[2])
  startp3<-round(77+shift_vec[3])
  endp3<-round(200+shift_vec[3])
  startp4<-round(201+shift_vec[3])
  endp4<-255
  ##
  Di <- init_settings$Di
  Dp <- init_settings$Dp
...
SEIRplot <- function(pars_estimate, file_name, init_settings, panel_B_R_ylim=6) {
  ...
  #
  abline(v = c(startp2,startp3,startp4, 255), lty = 3, lwd = 2, col = "darkgrey")
  text(c(startp2,startp3,startp4, 255), par()$usr[4], labels = mydate[c(startp2,startp3,startp4, 255)], col = "darkgrey", pos = 3, xpd = T)
  #
...
#2	2	0.669	1.944	0.736	-3.655	-3.573	2.099	1.914	-0.156	0.046
#with starting values for the MCMC to get the same displayed graph


startv=c(2,2,0.669,1.944,0.736,-3.655,-3.573,2.099,1.914,-0.156	,0.046)
startValue<-matrix(NA, 1,11)
...
#running with randomn starting values
ll=SEIRfitting(init_sets_list, randomize_startValue = T,run_id = "main_analysis", output_ret = T, skip_MCMC=F)
